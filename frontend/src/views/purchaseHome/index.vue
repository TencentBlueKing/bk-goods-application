    ...
    export default {
        data () {
            return {
                typeList: {}, // 种类列表
                type: '', // 种类
                last_type: '', // 用于保存上一次数据的种类
                bannerNum: 6, // 轮播图数量
                searchContent: '', // 搜索内容
                bannerPics: [], // 轮播图
                goodsList: '', // 商品列表
                isDropdownShow: false, // 种类组件参数
                currentPage: 1, // 当前页数（请求数据时提交的不是这个）
                param_page: 1, // 用于传参的当前页数
                pageCount: 0, // 总页数
                config: { // 组件参数
                    strokeWidth: 10,
                    bgColor: '#f0f1f5',
        ...
        methods: {
            ...
            getGoods () { // 获取指定商品信息
                if (this.type.id !== this.last_type.id) {
                    this.param_page = 1
                }
                this.$http.get(goodsUrl, {
                    params: {
                        ...
                        page: this.param_page // 指定页
                    }
                }).then(res => {
                    if (res && res.result === true) {
                        this.last_type = this.type
                        this.currentPage = this.param_page
